...
const QB_BASE_URL  = process.env.QUICKBOOKS_BASE_URL || "https://quickbooks.api.intuit.com";
...
const getAuthenticatedClient = async (clientId) => {
  ...
  if (!token) throw new Error(`No QuickBooks token found for user: ${clientId}`);
...
const refreshAndPersistTokenService = async (clientId) => {
  ...
  const { access_token, refresh_token, expires_in, x_refresh_token_expires_in, token_type } = authResponse.json;

  await prisma.quickbooksToken.update({
    where: { client_Id: clientId },
    data:  { access_token, refresh_token, expires_in, x_refresh_token_expires_in, token_type },
  });
...
const fetchQBReport = async (clientId, reportName, params = {}, retry = true) => {
  ...
  try {
    ...
    const message = error.response?.data?.Fault?.Error?.[0]?.Message || error.message;

    if (status === 401 && retry) {
      console.log(`[QB] 401 on ${reportName} — refreshing for user: ${clientId}`);
...
const getProfitAndLossService = async (clientId, { startDate, endDate, accounting = "Accrual" } = {}) => {
  ...
  const cogs          = Math.abs(findRowValue(rows, "Total Cost of Goods Sold") ?? findRowValue(rows, "Total COGS") ?? 0);
  ...
  return {
    ...
    cogs,
    grossProfit,
    grossMargin:       totalIncome ? parseFloat(((grossProfit / totalIncome) * 100).toFixed(2)) : 0,
    totalExpenses,
    netIncome,
    netMargin:         totalIncome ? parseFloat(((netIncome   / totalIncome) * 100).toFixed(2)) : 0,
    ebitda:            parseFloat(ebitda.toFixed(2)),
    ebitdaMargin:      totalIncome ? parseFloat(((ebitda      / totalIncome) * 100).toFixed(2)) : 0,
...
const getBalanceSheetService = async (clientId, { asOfDate } = {}) => {
  const params = asOfDate ? { date_macro: "Custom", end_date: asOfDate } : { date_macro: "Today" };
  const report = await fetchQBReport(clientId, "BalanceSheet", params);
  const rows   = report.Rows?.Row || [];

  const cashOnHand               = findRowValue(rows, "Total Bank Accounts") ?? findRowValue(rows, "Cash and Cash Equivalents") ?? findRowValue(rows, "Checking") ?? 0;
  ...
  const totalCurrentLiabilities  = findRowValue(rows, "Total Current Liabilities") ?? 0;
...
const getBudgetVsActualsService = async (clientId, { startDate, endDate } = {}) => {
  const params = startDate && endDate ? { start_date: startDate, end_date: endDate } : { date_macro: "This Fiscal Year" };
  ...
  const findBudgetRow = (rows, label) => {
    for (const row of rows) {
      ...
      if (colData?.[0]?.value?.toLowerCase().includes(label.toLowerCase()))
        return { actual: parseFloat(colData[1]?.value || "0"), budget: parseFloat(colData[2]?.value || "0") };
      if (row.Rows?.Row) { const found = findBudgetRow(row.Rows.Row, label); if (found) return found; }
    }
    return null;
  };

  const calcVariancePct = (actual, budget) =>
    !budget ? null : parseFloat((((actual - budget) / budget) * 100).toFixed(2));

  const revenue     = findBudgetRow(rows, "Total Income");
  const cogs        = findBudgetRow(rows, "Total Cost of Goods Sold") ?? findBudgetRow(rows, "Total COGS");
  ...
  return {
    ...
    varianceGrossProfitPct:       calcVariancePct(grossProfit?.actual, grossProfit?.budget),
 ...
 */
const getFinancialKPIsService = async (clientId, { startDate, endDate, asOfDate } = {}) => {
  ...
  const runwayMonths = netBurnRate > 0 ? parseFloat((bs.cashOnHand / netBurnRate).toFixed(1)) : null;
  const dayCount     = startDate && endDate ? Math.ceil((new Date(endDate) - new Date(startDate)) / 86400000) : 30;
  const dso          = pl.totalIncome > 0 ? parseFloat((bs.accountsReceivable / (pl.totalIncome / dayCount)).toFixed(1)) : null;
