...
const getValidAccessToken = async (clientId) => {
  ...
  if (isExpired) {
    console.log(`[HubSpot] Token expired for user: ${clientId} — refreshing...`);
 ...
 */
const fetchAllPages = async (clientId, objectType, params = {}, retry = true) => {
  const results = [];
  let after     = undefined;
  ...
  try {
    ...
    if (status === 401 && retry) {
      console.log(`[HubSpot] 401 on ${objectType} — refreshing token for user: ${clientId}`);
      const token = await getTokenRecord(clientId);
    ...
    throw new Error(`[HubSpot] ${objectType} fetch failed (${status}): ${message}`);
  }
};
...
const getLeadsCountService = async (clientId, { startDate, endDate }) => {
  const contacts = await fetchAllPages(clientId, "contacts", {
    ...
    filterGroups: JSON.stringify([{
      filters: [
        { propertyName: "lifecyclestage", operator: "IN",  values: ["marketingqualifiedlead", "salesqualifiedlead"] },
        { propertyName: "createdate",     operator: "GTE", value: new Date(startDate).getTime() },
        { propertyName: "createdate",     operator: "LTE", value: new Date(endDate).getTime()   },
      ],
    }]),
  });
...
const getPipelineMetricsService = async (clientId, { startDate, endDate }) => {
  ...
  // LEADS-4: deals won
  const dealsWon = periodDeals.filter((d) =>
  ...
  const calls = periodDeals.filter((d) =>
    d.properties.hs_activity_type === "CALL"
  ).length;
...
const getRevenueBreakdownService = async (clientId, { startDate, endDate }) => {
  ...
  const recurringRevenue = periodWon
    ...
    .reduce((sum, d) =>
      sum + parseFloat(d.properties.recurring_revenue_amount || d.properties.amount || "0"), 0
    );

  const oneTimeRevenue   = totalRevenue - recurringRevenue;
  const recurringPercent = totalRevenue > 0
...
const getClientMetricsService = async (clientId, { startDate, endDate }) => {
  ...
  for (const deal of closedWon) {
    ...
    revenueByCompany[companyId] =
      (revenueByCompany[companyId] || 0) + parseFloat(deal.properties.amount || "0");
  }
  ...
  const clientConcentration = totalWonValue > 0
  ...
  const dealVelocity = velocities.length > 0
  ...
  const prevStart = new Date(new Date(startDate).setMonth(new Date(startDate).getMonth() - 1));
  const prevEnd   = new Date(startDate);

  const prevWon = deals.filter((d) => {
    const closed = new Date(d.properties.closedate);
    return d.properties.dealstage === "closedwon" && closed >= prevStart && closed < prevEnd;
  });

  const currentCompanyIds = new Set(
    closedWon.map((d) => d.associations?.companies?.results?.[0]?.id).filter(Boolean)
  );
  const prevCompanyIds = new Set(
    prevWon.map((d) => d.associations?.companies?.results?.[0]?.id).filter(Boolean)
  );

  const retained = [...prevCompanyIds].filter((id) => currentCompanyIds.has(id)).length;
  const retentionRate = prevCompanyIds.size > 0
